...
if($rs){
  ...
  $sql="INSERT INTO `notifications`( `name`, `type`, `message`, `status`, `date`,`qty`) VALUES ('$fname','addstock','$name','unread',CURRENT_TIMESTAMP,'$qty')";
...
<table class="table table-bordered" id="tebol">
...
if($qtey<=100){
  //echo "STOCK AVAILABILITY IS IN LIMITATION";

  $sql="INSERT INTO `notifications`( `name`, `type`, `message`, `status`, `date`) VALUES ('$fname','stocklimit','$pro_name','unread',CURRENT_TIMESTAMP)";
    $notf=mysqli_query($conn,$sql);
  ...
  //echo "<p>YOUR RELEASE IS GREATER THAN STOCK AVAILABILITY!!!</p>";
    ...
    $sales=$reles*$prc;
     ...
     //insert sa sales
      ...
      if($run){
       ...
       $sql="INSERT INTO `notifications`( `name`, `type`, `message`, `status`, `date`, `qty`) VALUES ('$fname','stockrelease','$pro_name','unread',CURRENT_TIMESTAMP,'$reles')";
    $notf=mysqli_query($conn,$sql);
      ...
      if($run){
        ...
        if($rs){
          ...
          if($rss){
           ...
           // echo "Remaning stocks: $total"; //update sa stocks
         //   header("Location: stocks.php");
 $sql="INSERT INTO `notifications`( `name`, `type`, `message`, `status`, `date`) VALUES ('$fname','stockrelease','$pro_name','unread',CURRENT_TIMESTAMP)";
    $notf=mysqli_query($conn,$sql);
